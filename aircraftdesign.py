...
print(f'The Range of P_LD : [{round(P_land_90_per)},{round(P_land_110_per)}] N/m^2')
...
print(f"Range of P_PV=[{round(P_PV_range[1])},{round(P_PV_range[0])}] N/m^2")
...
print(f"The Range of P_Range=[{round(P_Range_roots[1])},{round(P_Range_roots[0])}] N/m^2")
...
if (engines==2):
    ...
    TOP=BFL/0.2196
    P_BFL=TOP*sigma*ClTO*T_W_TO
print(f"P_BFL={round(P_BFL,3)} N/m^2")
if (engines==2):
    TOP_var=0.2613
elif (engines==3):
    TOP_var=0.2387
...
print(f'The Range of P_BFL : [{round(P_BFL_min)}, {round(P_BFL_max)}] N/m^2 ')
...
print('\nWING LOADING')
print(f'The Range of Wing Loading Based on Landing Distance      : [{round(P_land_90_per)},{round(P_land_110_per)}] N/m^2')
print(f"The Range of Wing Loading Based on Prescribed Velocity   : [{round(P_PV_range[1])},{round(P_PV_range[0])}] N/m^2")
print(f"The Range of Wing Loading Based on Absolute Ceiling      : [{round(P_AC_range[1])},{round(P_AC_range[0])}] N/m^2")
print(f'The Range of Wing Loading Based on Rate of Climb         : [{round(P_R_C_roots[1])},{round(P_R_C_roots[0])}] N/m^2')
print(f"The Range of Wing Loading Based on Range                 : [{round(P_Range_roots[1])},{round(P_Range_roots[0])}] N/m^2")
print(f'The Range of Wing Loading Based on Balanced Field Length : [{round(P_BFL_min)},{round(P_BFL_max)}] N/m^2')
print(f'Optimum Wing Loading : [{round(P_maxofminrange)},{round(P_minofmaxrange)}] N/m^2\n')
...
if(M_Cruise<=0.35):
    ...
    sweep=np.arctan(np.tan(sweep_c4etw*np.pi/180)+(1/TR-1)/AR/(1/TR+1))*180/np.pi
    print(f'Sweep Angle : {sweep} deg')
    x=np.tan(sweep*np.pi/180)*y
    L_ac_LErc=x+0.25*Cw
    print(f'Distance from LE to A.C. (D_LEtoAC) : {L_ac_LErc} m')

    sweep_c2=np.arctan(np.tan(sweep_c4etw*np.pi/180)-4*0.25*(1-TR)/AR/(1+TR))
    ...
    def f(x):
        return 2*(Cr**2)/S
    x = sy.Symbol("x")
    a=sy.integrate(f(x), (x, 0, b*winginfus/2))
    def f(z):
        return 2/S*(Cr-(Cr-Ct)/((1-winginfus)*b/2)*(z-b*winginfus/2))**2 
    z = sy.Symbol("z")
    c=sy.integrate(f(z), (z, b*winginfus/2, b/2))

    Cw=round(a+c,3)
    print(f'Mean aerodynamic chord of the wing (Cw) : {Cw} m')
    ymac=round((Cr-Cw)*b/2*(1-winginfus)/(Cr-Ct)+winginfus*b/2,3)
    print(f'The spanwise location (ymac) where a chord of {Cw} m is located is (ymac) : {ymac} m')
    L_ac_LErc=round((ymac-b*winginfus/2)*np.tan(sweep_LEtw*np.pi/180)+Cw/4,3)
    ...
    sweep_c2=round((np.arctan(np.tan(sweep_c4etw*np.pi/180)-4*0.25*(1-TRe)/AR/(1+TRe)))*180/np.pi,3)
...
print(f'Distance between c.g. of airplane and aerodynamic centre of the horizontal tail (Lht) : {Lht} m')
...
print(f'Distance between c.g. of airplane and A.C. of the vertical tail (Lvt) : {Lvt} m')
Lvt_b=round(Lvt/b,3)
...
Wfus_sys=round((We_W0-(Wwing_Wo+Wht_Wo+Wvt_Wo+Wengine_Wo+Wlg_Wo)/100)*Wo*9.81,2)
...
Wpay_Wo=round(Wpay/Wo/9.81*100,2)
print(f"Wpay/Wo : {Wpay_Wo} %")
...
print(f"\nTakeoff Parameter")
V1=round(1.16*Vs,2)
print(f"Decision Speed (V1) : {V1} kmph")
V2=round(1.086*V1,2)
print(f"Rotation Speed (V2) : {V2} kmph")
...
##Gliding
print("\nGliding")
phi=round(np.arctan(1/L_Dcruise)*180/np.pi,3)
print(f"phi : {phi} deg")
